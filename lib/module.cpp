...
{
    return auto_catch_module(
        [in_features, out_features]() { return new torch::nn::LinearImpl(in_features, out_features); }, err);
}
    ...
    return auto_catch_module(
        [num_embeddings, embedding_dim, padding_idx]() {
            return new torch::nn::EmbeddingImpl(
                torch::nn::EmbeddingOptions(num_embeddings, embedding_dim).padding_idx(padding_idx));
        },
        err);
}
...
{
    auto_catch_void([m, device]() { m->to(torch::DeviceType(device)); }, err);
}

void module_to_scalar_type(char **err, module m, int8_t type)
{
    auto_catch_void([m, type]() { m->to(torch::Dtype(type)); }, err);
}
