...
optimizer new_adam_optimizer(char **err, tensor *params, size_t params_count, double lr, double beta1, double beta2,
                             double eps, double weight_decay)
{
    ...
    return auto_catch_optimizer(
        [params, params_count, lr, beta1, beta2, eps, weight_decay]() {
...
optimizer new_adamw_optimizer(char **err, tensor *params, size_t params_count, double lr, double beta1, double beta2,
                              double eps, bool amsgrad, double weight_decay)
{
    if (lr <= 0)
    {
        *err = strdup("lr must be positive");
    ...
    return auto_catch_optimizer(
        [params, params_count, lr, beta1, beta2, eps, amsgrad, weight_decay]() {
...
void free_optimizer(char **err, optimizer optm)
{
    auto_catch_void([optm]() { delete optm; }, err);
}
...
{
    auto_catch_void([optm, lr]() { optm->defaults().set_lr(lr); }, err);
}
...
{
    return auto_catch_optimizer_state(
        [optm]() {
...
{
    return auto_catch_size_t([state]() { return state->data.size(); }, err);
}
...
template <typename T> tensor get_adam_state(T ptr, size_t key)
{
    switch (key)
    {
    case 0:
...
template <typename T> void set_adam_state(T ptr, size_t key, tensor value)
{
    switch (key)
    {
    case 0:
